...
MoonPhaseImage.prototype._doDraw = function (viewAngle, hourAngle, extraAngles) {
	...
	ctx.fillStyle = 'rgba(' + brightness + ',' + brightness + ',' + saturation + ', 0.75)';
	...
	if ( Math.abs(viewAngle - MoonPhaseAnimation.DAWN) < 0.00001 || Math.abs(viewAngle - MoonPhaseAnimation.DUSK) < 0.00001 ) { 		
		if (isWaxing)
			ctx.fillRect(moonX-MoonPhaseImage.MOON_RADIUS, moonY-MoonPhaseImage.MOON_RADIUS, MoonPhaseImage.MOON_RADIUS, MoonPhaseImage.MOON_RADIUS*2);
		else
			ctx.fillRect(moonX, moonY-MoonPhaseImage.MOON_RADIUS, MoonPhaseImage.MOON_RADIUS, MoonPhaseImage.MOON_RADIUS*2);
		return;
	}	
